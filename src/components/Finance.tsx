    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <motion.div 
          ...
          className="bg-white dark:bg-[#151921] p-8 rounded-[32px] border border-slate-200 dark:border-slate-800/50 shadow-xl flex items-center gap-6"
        >
          ...
          <div>
            ...
            <h4 className="text-3xl font-black dark:text-white text-slate-900">${totalIncome.toFixed(2)}</h4>
          </div>
        </motion.div>
        ...
        >
          ...
          <div>
            ...
            <h4 className="text-3xl font-black dark:text-white text-slate-900">${totalExpense.toFixed(2)}</h4>
          </div>
        </motion.div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-1">
          <div className="bg-white dark:bg-[#151921] p-8 rounded-[32px] border border-slate-200 dark:border-slate-800/50 shadow-xl sticky top-8">
            <div className="flex items-center gap-4 mb-8">
              ...
              <h3 className="text-2xl font-bold dark:text-white text-slate-900">{t('addTransaction')}</h3>
            </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button 
                    ...
                    className={`py-4 rounded-2xl font-bold text-sm transition-all ${newRecord.type === 'Income' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-slate-50 dark:bg-[#0B0E14] text-slate-500 hover:bg-slate-100 dark:hover:bg-[#1D222B]'}`}
                  >
                    {t('income')}
                  </button>
                  <button 
                    ...
                    className={`py-4 rounded-2xl font-bold text-sm transition-all ${newRecord.type === 'Expense' ? 'bg-red-500 text-white shadow-lg shadow-red-500/20' : 'bg-slate-50 dark:bg-[#0B0E14] text-slate-500 hover:bg-slate-100 dark:hover:bg-[#1D222B]'}`}
                  >
                    {t('expense')}
                  </button>
                </div>
        ...
        <div className="xl:col-span-2">
          <div className="bg-white dark:bg-[#151921] rounded-[32px] border border-slate-200 dark:border-slate-800/50 shadow-xl overflow-hidden">
            <div className="p-8 border-b border-slate-200 dark:border-slate-800/50 flex items-center gap-4">
              <div className="w-12 h-12 bg-slate-50 dark:bg-[#0B0E14] rounded-2xl flex items-center justify-center text-slate-400">
                <History className="w-6 h-6" />
              </div>
              <h3 className="text-2xl font-bold dark:text-white text-slate-900">{t('transactionHistory')}</h3>
            </div>
              ...
              {loading ? (
                ...
                records.map((record) => (
                  <motion.div 
                    ...
                    animate={{ opacity: 1 }}
                    key={record.id} 
                    className="p-8 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-[#1D222B]/30 transition-colors group"
                  >
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        <p className="font-bold dark:text-white text-slate-900 text-lg">{record.description}</p>
